...
class AsyncRealtimeConnection:
    ...
    async def recv_bytes(self) -> bytes:
        ...
        if not isinstance(message, bytes):
            ...
            raise TypeError(
                f"Expected `.recv(decode=False)` to return `bytes` but got {type(message)}"
            )
    ...
    async def __aenter__(self) -> AsyncRealtimeConnection:
        ...
        **Warning**: You must remember to close the connection with `.close()`.
        ...
        if is_async_azure_client(self.__client):
            extra_query, auth_headers = await self.__client._configure_realtime(
    ...
    def __enter__(self) -> RealtimeConnection:
        ...
        **Warning**: You must remember to close the connection with `.close()`.
        ...
        if is_azure_client(self.__client):
            extra_query, auth_headers = self.__client._configure_realtime(
                self.__model, extra_query
            )
