...
# Se actualizó la ruta por defecto para que apunte a:
    ...
    if not USE_PLACEHOLDER:
        if not LOCAL_SCRIPT_PATH.is_file():
            raise FileNotFoundError(
                f"El script {LOCAL_SCRIPT_PATH} no existe. Verifica la configuración."
            )
        if not os.access(LOCAL_SCRIPT_PATH, os.X_OK):
            raise PermissionError(
                f"El script {LOCAL_SCRIPT_PATH} no tiene permisos de ejecución."
            )
    if LOCAL_TIMEOUT <= 0:
        raise ValueError(
