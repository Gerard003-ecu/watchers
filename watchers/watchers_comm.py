    ...
    try:
        with open(config_file, "r", encoding="utf-8") as f:
            ...
            if not isinstance(config_data, dict) or "watchers_wave" not in config_data:
                logging.warning("La configuración local no contiene la sección 'watchers_wave'. Se usarán valores por defecto.")
                return
            WATCHERS_CONFIG["watchers_wave_base_url"] = config_data["watchers_wave"].get("base_url", WATCHERS_CONFIG["watchers_wave_base_url"])
        ...
        logging.warning(f"No se pudo cargar la configuración local ({config_file}): {e}")

def send_event(file_path, suggestions):
    url = WATCHERS_CONFIG["watchers_wave_base_url"] + WATCHERS_CONFIG["endpoints"]["event"]
    event_data = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(f"Error al enviar evento: {response.status_code} {response.text}")
    except Exception as e:
        logging.error(f"Excepción al enviar evento: {e}")

def send_error(error_code, description, additional_data=None):
    url = WATCHERS_CONFIG["watchers_wave_base_url"] + WATCHERS_CONFIG["endpoints"]["error"]
    error_data = {
    ...
    try:
        ...
        if response.status_code == 200:
            logging.info(f"Reporte de error enviado exitosamente: {error_data['error_id']}")
        else:
            logging.error(f"Error al enviar reporte de error: {response.status_code} {response.text}")
    except Exception as e:
        logging.error(f"Excepción al enviar reporte de error: {e}")

def update_config_periodically(interval=60):
    url = WATCHERS_CONFIG["watchers_wave_base_url"] + WATCHERS_CONFIG["endpoints"]["config"]
    while True:
        try:
            ...
            if response.status_code == 200:
                ...
                if config_json.get("status") == "success":
                    ...
                    logging.error("Error en respuesta de configuración: " + config_json.get("message", ""))
            else:
