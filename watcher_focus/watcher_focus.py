...
def simulate_watcher_focus(dt=0.01, total_time=30.0):
    ...
    while t < total_time:
        ...
        if int(t * 100) % 10 == 0:
            ...
            logging.info(f"t={t:.2f} | x={x:.3f} | y={y:.3f} | z={z:.3f} | phase={indicators['phase']:.3f} | z_error={indicators['z_error']:.3f}")
        time.sleep(dt)
...
def actualizar_estado_watcher_focus():
    ...
    if estado:
        ...
        logging.info("Celdas críticas detectadas:\n" + json.dumps(celdas_criticas, indent=4))
    else:
...
def main():
    ...
    sim_thread = threading.Thread(target=simulate_watcher_focus, kwargs={"dt": 0.01, "total_time": 30.0})
